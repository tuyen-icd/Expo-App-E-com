...
const Notification = (props: NotificationProps) => {
    ...
    const [dataNotificationCurrent, setDataNotificationCurrent] = useState<any>(props?.dataNotification);

    const handleDelete = async (notificationId: number) => {
        setIsLoading(true);
        console.log("notificationId", notificationId);
        try {
            const response = await axios.delete(`http://localhost:5000/user/delete-notification/${notificationId}`);
            if (response.data.success) {
                const filteredNotifications = props.dataNotification.filter(
          ...
          {dataNotificationCurrent == null || dataNotificationCurrent == '' ? (
              ...
              <View>
                  <FlatList
                      ...
                      renderItem={({ item }) => (
                          <SwipeableNotification notification={item} onDelete={handleDelete} />
                      )}
                  />
