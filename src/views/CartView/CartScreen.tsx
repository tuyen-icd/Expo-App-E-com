...
const CartScreen = () => {
  ...
  const prices = cartRedux?.items.map(
    (item: any) => item?.price * item?.quantity
  );
  const toltalPrice = prices?.reduce(
    (acc: any, currentPrice: any) => acc + currentPrice,
    0
  );
    ...
    <View style={{ flex: 1, backgroundColor: AppEComm.color.white }}>
      ...
      {items?.length === 0 || items === undefined ? (
        <View style={[styles.imgEmptyCart]}>
          ...
          >
            Thank you for Cart using Cart-Ecom
          </Text>
          <Spacer height={50} />
