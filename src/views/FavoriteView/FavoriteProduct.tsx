...
const FavoriteProduct = () => {
  ...
  const handleDelete = (productId: number) => {
    setIsLoading(true);
    setTimeout(() => {
      const filterProduct = favoriteRedux?.items.filter((item: any) => item?.id !== productId);
        ...
        favoriteRedux?.items.length == 0 || !favoriteRedux?.items  ? (
          <View style={[styles.imgEmptyCart]}>
            ...
            <Text style={{
              ...
              lineHeight: 15,
              paddingVertical: 8
            }}>Thank you for Favorite using ICD-Ecom</Text>
            <Spacer height={50} />
...
const SwipeableProductItem: FC<SwipeableProductItemProps> = ({ product, onDelete }) => {
  const swipeoutProps = {
      right : [
        {
          text: 'Delete',
    ...
    <Swipeout {...swipeoutProps}>
      <View style={styles.borderItem}>
        <View style={[defaultStyle.flexRowStart]}>
          ...
          <View style={[defaultStyle.flexJustify, { flexDirection: 'column', paddingLeft: widthPixel(20) }]}>
            <View style={[defaultStyle.flexJustify]}>
              <Text style={styles.titleItem}>{product?.title.charAt(0).toUpperCase() + product?.title.slice(1)}</Text>
              <Text style={styles.txtPriceDefault}>${Math.round(product?.price / (1 - product?.discountPercentage / 100))}</Text>
            </View>
            <View style={[{ width: 150, alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }]}>
              <Text style={styles.txtDiscountPercentage}> - ${Math.round((product?.price / (1 - product?.discountPercentage / 100)) - product?.price)} Off</Text>
